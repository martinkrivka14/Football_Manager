...
type TeamStats = {
  ...
  teamOverall: number; played: number; wins: number; draws: number;
  losses: number; goalsFor: number; goalsAgainst: number; points: number; goalDifference: number;
};

type LeagueData = {
  id: string; name: string; teams: TeamStats[];
};

export default function LeagueClient({ leagues, userLeagueId, saveId }: { leagues: LeagueData[], userLeagueId: string | null, saveId: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a111a] p-6 rounded-xl border border-[#1a2533]">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button 
          onClick={handleCalculateOVR} disabled={isCalculating}
          className="text-xs bg-slate-700 text-slate-300 px-3 py-1 rounded hover:bg-slate-600"
        >
